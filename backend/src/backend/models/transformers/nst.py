...
from backend.layers import NSTEncoder
...
@register_model("sprint.transformers.nst")
class NonStationaryTransformer(QuantileModel):
    def __init__(
        self,
        *,
        n_features: int,
        ...
        self.stationarization = SeriesStationarization(n_features)
        ...
        self.encoder = nn.ModuleList(
            [NSTEncoder(embed_dim, n_heads, dropout=dropout) for _ in range(n_layers)]
        )
    ...
    def forward(self, x: torch.Tensor, ticker: torch.Tensor) -> torch.Tensor:
        ...
        x_stationary, x_mean, x_std = self.stationarization(x)

        x = x_stationary.transpose(-1, -2)
        x = self.input_proj(x)
        x = self.pos_enc(x)
